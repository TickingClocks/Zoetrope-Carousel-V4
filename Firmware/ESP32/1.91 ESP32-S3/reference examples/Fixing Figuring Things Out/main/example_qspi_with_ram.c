...
#include "ui.h" //lvgl squareline generated ui
#include "heartbeat.h" //heartbeat led code
#include "uart_protocol.h" //uart rx data structure
#include "platter_ui.h" //platter ui update functions
#include "ui_highlight.h" //ui highlight functions
#include "auto_scroll.h" //auto scroll function
...
void app_main(void)
{
    ...
    //indev_drv.disp = disp; //idk why this broke -kadin
    ...
    if (example_lvgl_lock(-1)) 
    {
        ...
        ui_init();         // ← SquareLine entry point
        heartbeat_init(); //heartbeat led code
        heartbeat_start(); //start heartbeat led
        uart_rx_init(); //uart communication init
        platter_ui_init(); //platter ui update functions
        auto_scroll_start(); /* Auto-scroll past splash image after 4 seconds */
        ui_highlight_init(ui_Container1); //ui highlight functions
